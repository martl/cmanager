...
public class GeocacheComparatorTest {
    ...
    private void add(
            ...
            String code_gc) {
        try {
            final Geocache g1 = new Geocache(code1, name1, new Coordinate(coords1), d1, t1, type1);
            g1.setOwner(owner1);
            g1.setContainer(container1);
            g1.setArchived(archived1);
            g1.setAvailable(available1);

            final Geocache g2 = new Geocache(code2, name2, new Coordinate(coords2), d2, t2, type2);
            g2.setOwner(owner2);
    ...
    public void testNotMatching() {
        ...
        for (final Geocache[] tuple : not_matching) {
            final Geocache gc = tuple[0];
            final Geocache oc = tuple[1];
            if (GeocacheComparator.similar(gc, oc)) {
